...
public class SoapMsgFilter implements Filter {
  //Bug 27017391, after saml has been upgraded, the saml xml name sapce has been changed from saml to saml2.
  private final static String SAMLXMLNS = "saml2";
  ...
  private String requestSignatureValues[] = null;
  private String requestSignatureReferences[] = null;
  private String responseSignatureReferences[] = null;
  ...
  public void doFilter(ServletRequest req,
    ...
    try {
      ...
      if (requestData != null && requestData.length() > 0) {
        ...
        if (requestData.indexOf("Fault") == -1) {
          ...
          else if (soapAction != null && soapAction.indexOf("Eas") != -1 && soapAction.indexOf("Mismatch") == -1 && soapAction.indexOf("V20") == -1 && soapAction.indexOf("V11") == -1)
            handleSingnatureMethod(requestData);
        }
      }
      ...
      if (requestData != null && requestData.length() > 0) {
        ...
        if (responseData.indexOf("Fault") != -1) {
          ...
          if (soapAction.indexOf("InboundOnly") == -1) {
            handleSingnatureMethod(responseData);
          }
        }
      }
    ...
    private void handleRequestSamlAssertionV11 (String samlVersion, String samlTokenVersion) throws Exception {
      ...
      start = requestData.indexOf("<Subject ", start) > 0 || requestData.indexOf("<Subject>", start) > 0 ? 1 : -1;
      if (start == -1) {
      ...
      start = requestData.indexOf("<SubjectConfirmation ", start) > 0 || requestData.indexOf("<SubjectConfirmation>", start) > 0 ? 1 : -1;
      if (start == -1) {
      ...
      start = requestData.indexOf("<ConfirmationMethod ", start) > 0 || requestData.indexOf("<ConfirmationMethod>", start) > 0 ? 1 : -1;
      if (start == -1) {
    ...
    private void handleRequestReplaceSamlAssertion () throws Exception {
      int start = 0;
      int end = 0;
      ...
      String part1 = requestData.substring(0, start);
      String part2 = requestData.substring(end + ("</" + SAMLXMLNS + ":Assertion>").length(), requestData.length());

      if (samlAssertionReplace != null)
        requestData = part1 + samlAssertionReplace + part2;

      trace(" Modified request with new assertion");
    ...
    private void handleExpectedFaultInResponse () throws Exception {
      ...
      //return createSoapFaultStrWithMessage(message_faultstring, message_faultcode);
    }
