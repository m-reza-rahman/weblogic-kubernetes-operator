...
| `rollingRestartTimeout` | integer | Number of seconds after which a rolling restart of a cluster will timeout.  If a rolling restart of a cluster has not completed within the specified timeout duration, the Domain condition will be set to a FAILED state.  A value of 0 means a rolling restart will not timeout. Defaults to 0. |
