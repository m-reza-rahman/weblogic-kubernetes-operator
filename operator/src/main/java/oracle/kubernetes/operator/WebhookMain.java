...
public class WebhookMain extends BaseMain {
  ...
  Step createCRDRecheckSteps() {
    Step crdRecheckSteps = Step.chain(createDomainCRDPresenceCheck(), createClusterCRDPresenceCheck());
    try {
      ...
      if (crdMissingOrChanged(clusterCrdResourceVersion, delegate.getClusterCrdResourceVersion())) {
        crdRecheckSteps = Step.chain(createClusterCrdStep(delegate.getProductVersion()), crdRecheckSteps);
      }
      if (crdMissingOrChanged(domainCrdResourceVersion, delegate.getDomainCrdResourceVersion())) {
        crdRecheckSteps = Step.chain(createDomainCrdStep(delegate.getProductVersion(), new Certificates(delegate)),
            crdRecheckSteps);
      }
